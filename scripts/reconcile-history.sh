...
#
# Usage: ./scripts/reconcile-history.sh [--dry-run] [--yes]
#   --dry-run  Show what would be done without making changes
#   --yes      Skip interactive confirmation, auto-proceed with reconciliation
...
for arg in "$@"; do
    case $arg in
        ...
        --yes|-y)
            AUTO_YES=true
            echo -e "${CYAN}🚀 AUTO MODE - Will proceed with reconciliation automatically${NC}"
            ;;
    esac
done
...
analyze_commit() {
    ...
    echo -e "${BLUE}  📄 Files changed:${NC}" >&2
    echo "$files_changed" | head -10 | sed 's/^/      /' >&2
...
## Your Task
Write ONLY the commit message. No preamble, no explanation, no "Here's the message:" - just output the commit message directly.

Requirements:
1. First line: the original message (summary)
2. Blank line
3. "## What Changed" section with 2-4 bullet points
4. "## Why" section with architectural/strategic reasoning
5. "## Philosophy" section if applicable
...
Keep it concise. No more than 20 lines total. NO PREAMBLE TEXT.
EOF
)
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        echo -e "${YELLOW}  [DRY RUN] Would call Claude CLI here${NC}" >&2
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    if [[ "$TREES_MATCH" == "false" ]]; then
        ...
        # Check for auto-yes mode or prompt for confirmation
        PROCEED=false
        if [[ "$AUTO_YES" == "true" ]]; then
            echo -e "${CYAN}Auto-proceeding with reconciliation (--yes flag)...${NC}"
            PROCEED=true
        else
